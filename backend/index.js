...
const { connectDB, ensureDemoUsers } = require('./db');
...
// Connect to MongoDB Atlas and seed demo users when the database is empty
connectDB()
  .then(async () => {
    try {
      await ensureDemoUsers();
    } catch (error) {
      console.error('Startup demo user seeding failed:', error.message);
    }
  })
  .catch((error) => {
