...
import { fetchReports, fetchVehicles, fetchCollections } from '../../lib/api';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!token) return;
      try {
        const r = await fetchReports(token);
        setReports(r);
      } catch (err) {
        console.warn('Could not fetch reports from API, using mock data');
      }
      try {
        const v = await fetchVehicles(token);
        setVehicles(v);
      } catch (err) {
        console.warn('Could not fetch vehicles from API, using mock data');
      }
      try {
        const c = await fetchCollections(token);
        setCollections(c);
      } catch (err) {
        console.warn('Could not fetch collections from API, using mock data');
      }
    };
    load();
  }, [token]);
  ...
  const stats = {
    ...
    pendingReports: reports.filter(r => r.status === 'received').length,
        ...
        <div className="glass-panel p-6 rounded-lg shadow-sm border border-white/10">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-white">{stats.completedCollections}</p>
            </div>
